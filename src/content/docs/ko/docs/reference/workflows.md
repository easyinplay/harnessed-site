---
title: 워크플로 레퍼런스
description: 현재 릴리스의 harnessed에 포함된 29개 조합 가능한 워크플로.
---

harnessed는 네임스페이스로 계층화된 29개 워크플로를 제공합니다: super-master 1개, 단계 master 5개(Discuss · Plan · Task · Verify · Ship), 서브워크플로 20개, 독립 워크플로 2개.

29개 워크플로 —— 하나의 super-master가 다섯 stage master와 그 서브들로 부채꼴로 퍼지고, 여기에 독립 워크플로 2개가 더해집니다:
...
| `/task-deliver` | 서브워크플로 | harnessed 자체 완료 게이트(`harnessed checkpoint complete`). 문자 그대로의 `COMPLETE`가 나올 때까지 실행. 풀스택 조율 시 Agent Teams 조건부 발동.                            |

## Verify 단계

| 명령                     | 범위         | capability                                                                                                                       |
| ------------------------ | ------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| `/verify`                | 단계 master  | 시나리오 플래그에 따라 최대 11가지 하위 검사 배분.                                                                                |
...
| `/verify-second-opinion`  | 서브 워크플로 | 마지막 release tag 이후 diff에 대한 교차 모델 세컨드 오피니언. 엔진이 실제로 읽는 면을 건드릴 때 실행됩니다.                                      |
...
| 명령   | 범위 | 기능                                                                                  |
| ------ | ---- | ------------------------------------------------------------------------------------- |
| `/tdd` | 규율 | Red → green → refactor. 업스트림 `superpowers:test-driven-development`을 조합(mattpocock `/tdd`는 대체). |

완료 약속은 더 이상 업스트림 래퍼가 아닙니다. 4.36.0부터 harnessed 자체 게이트인 `harnessed checkpoint complete <sub>`가 담당하며(ADR 0039), 그래서 `/ralph-loop`과 예전 `/execute-task` 진입점은 사라졌습니다.
